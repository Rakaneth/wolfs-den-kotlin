...
class Entity (
    ...
    val atk: Int
        get() = (armor?.atk ?: 0) + (mh?.atk ?: 0) + (oh?.atk ?: 0) + (trinket?.atk ?: 0)
    val dfp: Int
        get() = (armor?.dfp ?: 0) + (mh?.dfp ?: 0) + (oh?.dfp ?: 0) + (trinket?.dfp ?: 0)
    val dmg: Int
        get() = (armor?.dmg ?: 0) + (mh?.dmg ?: 0) + (oh?.dmg ?: 0) + (trinket?.dmg ?: 0)
    val sav: Int
        get() = (armor?.sav ?: 0) + (mh?.sav ?: 0) + (oh?.sav ?: 0) + (trinket?.sav ?: 0)
    val atkDly: Int
        get() = maxOf(11 - maxOf(mh?.dly ?: 0, oh?.dly ?: 0), 1)
    val movDly: Int
        get() = maxOf(11 - (armor?.dly ?: 0), 1)
