    ...
    PLAY(null) {
        override fun enter(entity: PlayScreen?) {
            entity!!.input = SquidInput({ key, alt, ctrl, shift ->
                                            ...
                                            when (key) {
                                                SquidInput.UP_ARROW -> CommandProcessor.process(player, "move",
                                                                                                Direction.UP)
                                                SquidInput.UP_RIGHT_ARROW -> CommandProcessor.process(player, "move",
                                                                                                      Direction.UP_RIGHT)
                                                SquidInput.RIGHT_ARROW -> CommandProcessor.process(player, "move",
                                                                                                   Direction.RIGHT)
                                                SquidInput.DOWN_RIGHT_ARROW -> CommandProcessor.process(player, "move",
                                                                                                        Direction.DOWN_RIGHT)
                                                SquidInput.DOWN_ARROW -> CommandProcessor.process(player, "move",
                                                                                                  Direction.DOWN)
                                                SquidInput.DOWN_LEFT_ARROW -> CommandProcessor.process(player, "move",
                                                                                                       Direction.DOWN_LEFT)
                                                SquidInput.LEFT_ARROW -> CommandProcessor.process(player, "move",
                                                                                                  Direction.LEFT)
                                                SquidInput.UP_LEFT_ARROW -> CommandProcessor.process(player, "move",
                                                                                                     Direction.UP_LEFT)
                                                ...
                                                't' -> {
                                                    entity.stage.addAction(
                                                        PanelEffect.GlowBallEffect(entity.mapLayers, Coord.get(40, 20),
                                                                                   Coord.get(35, 20), 5))
                                                }
                                                in SquidInput.F1..SquidInput.F10 -> {
                                                    ...
                                                    val theSkill = player.skillStack!!.skillTable.getOrNull(fMap[key]!!)
                                                            ?.second
                                                    ...
                                                    if (theSkill?.isAvailable == true) {
                                                        if (theSkill.targeting == true) {
                                                            ...
                                                            CommandProcessor.process(player, "skill",
                                                                                     player.pos!!.coord)
                                                    } else {
